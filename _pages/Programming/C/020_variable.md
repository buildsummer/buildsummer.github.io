...
---
###5.3 변수
* <font color = 'ff8000'>**로컬 변수**</font>는 중괄호 {}기준
  * 함수고 뭐고 상관없이 그냥 중괄호 안에 있으면 로컬변수임
    * 괄호 안과 밖의 변수 이름이 같더라도 가리키는 메모리 위치가 다름(바깥의 변수는 임시적으로 숨겨짐)
  * 영역이 끝나면 파괴됨
* 글로별 변수는 main 바깥에 생성하면 그게 글로벌
  * 글로벌 변수는 정의 시 자동으로 0으로 초기화
  * 글로벌변수는 메모리의 <font color = 'ff8000'>**데이터세그먼트**</font>에 할당됨
  * main 함수 호출보다 먼저 진행

* <font color = 'ff8000'>**정적 변수 static variable**</font>
  * 로컬에서 선언해도 파괴되지 않는 변수
  * 함수를 실행하지 않더라도 메모리에 존재하며 글로벌 변수처럼 작동함
    * <font color = 'ff8000'>**static int a = 2;를 함수에 넣어 함수를 여러번 실행하더라도, 변수 초기화는 한 번만 진행됨**</font>
...
* void 포인터
  * void*
  * 없는 것의 포인터지만, 정의되어있지 않은 데이터형이므로, <font color = 'ff8000'>**어떤 데이터형의 포인터라도 담을 수 있음**</font>
  * 온전히 주소값의 <font color = 'ff8000'>**보관 역할**</font>만 수행
